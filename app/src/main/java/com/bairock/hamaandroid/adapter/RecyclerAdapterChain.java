...
public class RecyclerAdapterChain extends RecyclerView.Adapter<RecyclerAdapterChain.ViewHolder> {
    ...
    public RecyclerAdapterChain(Context context) {
        this.mInflater = LayoutInflater.from(context);
    }

    @Override
    public int getItemCount() {
        return HamaApp.DEV_GROUP.getChainHolder().getListLinkage() == null ? 0 : HamaApp.DEV_GROUP.getChainHolder().getListLinkage().size();
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.setData(HamaApp.DEV_GROUP.getChainHolder().getListLinkage().get(position));
    }
