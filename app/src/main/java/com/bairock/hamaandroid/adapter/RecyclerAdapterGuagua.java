...
public class RecyclerAdapterGuagua extends RecyclerView.Adapter<RecyclerAdapterGuagua.ViewHolder> {

    private LayoutInflater mInflater;

    public RecyclerAdapterGuagua(Context context) {
        this.mInflater = LayoutInflater.from(context);
    }

    @Override
    public int getItemCount() {
        return HamaApp.DEV_GROUP.getGuaguaHolder().getListLinkage() == null ? 0 : HamaApp.DEV_GROUP.getGuaguaHolder().getListLinkage().size();
    }

    @NonNull
    @Override
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(mInflater.inflate(R.layout.adapter_linkage_holder, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.setData(HamaApp.DEV_GROUP.getGuaguaHolder().getListLinkage().get(position));
    }
