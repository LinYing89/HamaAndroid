...
public class RecyclerAdapterTiming extends RecyclerView.Adapter<RecyclerAdapterTiming.ViewHolder>{

    private LayoutInflater mInflater;

    public RecyclerAdapterTiming(Context context) {
        this.mInflater = LayoutInflater.from(context);
    }

    @Override
    public int getItemCount() {
        return HamaApp.DEV_GROUP.getTimingHolder().getListLinkage() == null ? 0 : HamaApp.DEV_GROUP.getTimingHolder().getListLinkage().size();
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.setData(HamaApp.DEV_GROUP.getTimingHolder().getListLinkage().get(position));
    }
